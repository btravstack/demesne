...
- [**demesne**](./core/) — the core `Tag` / `Context` / `Layer` / `Scope` types, the
  ...
  `Layer.acquireRelease` / `Layer.member` / `Layer.class` / `Layer.fromService` constructors,
  the `Layer.merge` / `Layer.provideTo` / `Layer.collect` / `Layer.onStart` / `Layer.onStop`
  combinators, the `Layer.describe` / `Layer.toDot` introspection aids, and
...
mints a self-injecting service class (a `Tag`; `Layer.fromService` builds its layer).
